...
case $CHOICE in
        ...
        2)  
            ...
            sleep 5
            pg_dump -v -h $dbIP -U postgres $reportdbname -f /dacx/"$directory"/appDbBk/$reportdbname_`date +%F`.sql 
            echo -e "\nReportsDatabase backed up to /dacx/"$directory"/appDbBk/"
            sleep 5

            echo "executing art configuration DB backup script now.."
            sleep 5
            pg_dump -v -h $dbIP -U postgres art_configuration_db -f /dacx/"$directory"/appDbBk/$artconfigurationdb_`date +%F`.sql 
        ...
        3)  directory=$(ls /dacx/ | grep R)
            ...
            sleep 2
            scp -vr root@$artIP:/ameyo_mnt/dacx/var/ameyo/dacxdata/art.product.com/conf /dacx/$directory/artBk/
            sleep 5
            scp -vr root@$artIP:/ameyo_mnt/dacx/ameyo/art.product.com/plugins /dacx/$directory/artBk/
            sleep 5
            ...
            sleep 2psql -U postgres -d database_name -c
...
case $CHOICE in
        ...
        rpm -Uvh "$directory"/release/ameyo-server*.rpm
        sleep 5
        ...
        if [ "$u2"="y" ]; then
        read -p "please enter art server password:"password
        sshpass -p $password ssh root@$artserverIP "mkdir /dacx/artrelease"
        sleep 2
        scp -vr "$directory"/release/ameyo-art* root@$artserverIP:/dacx/artrelease/
        sleep 2
        echo "stopping art server"
        sshpass -p $password ssh root@$artserverIP "ameyoctl service ameyoart stop"
        sleep 5
        sshpass -p $password ssh root@$artserverIP "rpm -Uvh dacx/artrelease/ameyo-art*.rpm"
        sleep 5
        ...
        sshpass -p $password ssh root@$artserverIP "ameyoctl service ameyoart start"
        else
        "exiting.."
        fi       
        ;;
